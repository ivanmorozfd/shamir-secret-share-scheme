...
def random_polynomial(degree, intercept, upper_bound):
    ...
    for i in range(degree):
        random_coeff = Crypto.Util.number.getRandomInteger(100)
...
def get_polynomial_points(coefficients, num_points, prime):
    ...
    for x in range(1, num_points + 1):
        ...
        y = coefficients[0]
        # calculate each  and add it to y, using modular math
...
def modular_lagrange_interpolation(x, points, prime):
    ...
    for i in range(len(points)):
        ...
        lagrange_polynomial = numerator * pow(denominator, -1, prime)
...
file_in = open("res/in.json",'r');
json_in = json.load(file_in)
# (k,n) schema

k = json_in["k"]
n = json_in["n"]
secret_string = json_in["secret"]


secret = SecretSharer()
shares = secret.split_secret(secret_string, k, n)

json_out = dict()

json_out["parts"] = shares
json_out["k"] = k

file_out = open("res/out.json","w")
file_out.write(json.dumps(json_out))
file_out.close()

print(secret.recover_secret(shares))

file_in_recover = open("res/out.json","r")
json_in = json.load(file_in_recover)

shares = json_in["parts"]

shares.pop()
shares.pop()

secret_string = secret.recover_secret(shares)
print(secret_string)
